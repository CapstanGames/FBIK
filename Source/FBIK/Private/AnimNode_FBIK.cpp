...
FHashBuilder FAnimNode_FBIK::BuildHash(const FBoneContainer& BoneContainer) const
{
	...
	for (const FAhoyPBIKEffector& Eff : Effectors.Effectors)
	{
		Hash << Eff.Bone;
	}

	for (const FAhoyPBIKBoneSetting& Set : BoneSettings.BoneSettings)
	{
		Hash << Set.Bone;
	}
...
void FAnimNode_FBIK::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	for (int32 BSI = 0; BSI < BoneSettings.BoneSettings.Num(); ++BSI)
	{
		...
		if (AhoyPBIK::FAhoyBoneSettings* BSettings = WorkData.Solver.GetBoneSettings(Idx))
	...
	for (int32 E = 0; E < Effectors.Effectors.Num(); ++E)
	{
		...
		const FAhoyPBIKEffector& Eff = Effectors.Effectors[E];
		AhoyPBIK::FAhoyEffectorSettings S;
		S.Weight = Eff.Weight;
